...
fn train(client: &mut ClassifierClient) {
                  ...
                  "家斉", "家慶", "家定", "家茂"]
    {
        let mut d = Datum::new();
        d.add_string("name", n);
        train_data.push(LabeledDatum { label: "徳川".to_string(),
                                       data: d, });
    }
                  ...
                  "義澄", "義稙", "義晴", "義輝", "義栄"]
    {
        let mut d = Datum::new();
        d.add_string("name", n);
        train_data.push(LabeledDatum { label: "足利".to_string(),
                                       data: d, });
    }
                  ...
                  "宗宣", "煕時", "基時", "高時", "貞顕"]
    {
        let mut d = Datum::new();
        d.add_string("name", n);
        train_data.push(LabeledDatum { label: "北条".to_string(),
                                       data: d, });
    }
...
fn predict(client: &mut ClassifierClient) {
    ...
    for n in vec!["慶喜", "義昭", "守時"] {
        ...
        let ref shogun_name = res[0].iter_mut()
                                    .max_by_key(|x| (x.score * 100.) as i64)
                                    .unwrap()
                                    .label;
